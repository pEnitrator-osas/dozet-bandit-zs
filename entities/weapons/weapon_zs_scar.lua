...
SWEP.TranslateName = "weapon_scar_name"
SWEP.TranslateDesc = "weapon_scar_desc"
SWEP.Slot = 2
SWEP.SlotPos = 0
...
function SWEP:GetCone()
	return BaseClass.GetCone(self) * (1 - self:GetHitStacks()/25)
end
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
	...
	if SERVER then
		...
		if ent:IsValid() then
			...
			if ent:IsPlayer() and wep:GetHitStacks() >= 15 then
